# Architecture:
#   ENCODER  : ResNet34 (pretrained) — unchanged skip connections at /2,/4,/8,/16,/32
#   DECODER  : Inception-style ConvBlocks (multi-scale 1x1 + 3x3 + 5x5 branches)
#   ATTENTION: Spatial Attention Gate on every skip connection before concat
...
OUTPUT_DIR = os.path.join(BASE_DIR, "outputs_resnet_inc_spattn_256")
...
class ISICDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img is None:
            raise RuntimeError(f"Failed to read image: {self.X[idx]}")
        ...
        if mask is None:
            raise RuntimeError(f"Failed to read mask: {self.y[idx]}")
...
def save_history_plot(history, out_path):
    ...
    plt.subplot(1, 3, 2)
    plt.plot(history["dice"], label="train"); plt.plot(history["val_dice"], label="val")
    plt.title("Dice"); plt.legend(); plt.grid(True)
    plt.subplot(1, 3, 3)
    plt.plot(history["iou"], label="train"); plt.plot(history["val_iou"], label="val")
...
def save_prediction_preview(model, loader, device, out_path, n=4, thr=0.5):
    model.eval()
    xb, yb = next(iter(loader))
    xb = xb[:n].to(device); yb_np = yb[:n].numpy()
    with torch.no_grad():
        logits = model(xb)
    ...
    for i in range(n):
        ...
        gt = yb_np[i, 0]; pr = probs[i, 0]; pb = preds[i, 0]
...
# ------------------------------------------------------------------
# 6a) Inception-style ConvBlock  (used in DECODER + bottleneck)
#
#  Three parallel branches fused via concat → 1x1 projection:
#    b1: 1x1              — point-wise / channel mixing
#    b2: 1x1 → 3x3       — local context
#    b3: 1x1 → 3x3 → 3x3 — larger receptive field (≈5x5, cheaper)
#
#  All branches produce (out_ch // 3) channels;
#  a final 1x1 conv fuses the concat back to out_ch.
# ------------------------------------------------------------------
class InceptionConvBlock(nn.Module):
    def __init__(self, in_ch: int, out_ch: int):
        super().__init__()
        b = max(out_ch // 3, 8)   # branch width
        ...
        # branch 3 — 1×1 → 3×3 → 3×3  (receptive field ≈ 5×5)
        ...
        # fuse all branches → out_ch
        self.fuse = nn.Sequential(
            nn.Conv2d(b * 3, out_ch, 1, bias=False),
...
# ------------------------------------------------------------------
# 6b) Spatial Attention Gate
#
#  Classic additive attention (Oktay et al. 2018) applied on the
#  SKIP connection before concatenation with the upsampled decoder:
#
#    g  = gating signal (from decoder, coarser scale)
#    x  = skip feature  (from encoder, finer scale)
#
#  alpha = sigmoid( W_x(x) + W_g(g_upsampled) + b )  → spatial map
#  out   = alpha * x                                  → attended skip
# ------------------------------------------------------------------
class SpatialAttentionGate(nn.Module):
    ...
    def forward(self, x, g):
        """
        x : skip feature  [B, x_ch,  H, W]
        g : gate signal    [B, g_ch, H', W']  (may be coarser)
        """
        ...
        return x * att                # broadcast over channels

# ------------------------------------------------------------------
# 6c) Decoder UpBlock
#
#  1. Transpose conv to upsample decoder features
#  2. Spatial attention on the skip connection  ← NEW
#  3. Concatenate attended-skip + upsampled decoder
#  4. Inception ConvBlock to refine               ← INCEPTION
# ------------------------------------------------------------------
class UpBlock(nn.Module):
    def __init__(self, in_ch: int, skip_ch: int, out_ch: int):
        super().__init__()
        self.up  = nn.ConvTranspose2d(in_ch, out_ch, kernel_size=2, stride=2)
        self.att = SpatialAttentionGate(x_ch=skip_ch, g_ch=out_ch)
    ...
    def forward(self, x, skip):
        ...
        skip = self.att(skip, x)          # attend skip with gating signal x
...
# 7) FULL MODEL
# =========================
class ResNetInceptionUNet(nn.Module):
    """
    Encoder : ResNet34 (pretrained ImageNet)
    Decoder : Inception ConvBlocks at every decoder stage
    Attention: Spatial Attention Gates on all skip connections
    """
    def __init__(self, pretrained: bool = True):
        super().__init__()
        enc = models.resnet34(weights=models.ResNet34_Weights.DEFAULT if pretrained else None)

        # ---- ENCODER (ResNet34, frozen structure) ----
        self.stem = nn.Sequential(enc.conv1, enc.bn1, enc.relu)  # out: 64 ch,  /2
        self.pool = enc.maxpool                                   # out: 64 ch,  /4
        self.e1   = enc.layer1                                    # out: 64 ch,  /4
        self.e2   = enc.layer2                                    # out: 128 ch, /8
        self.e3   = enc.layer3                                    # out: 256 ch, /16
        self.e4   = enc.layer4                                    # out: 512 ch, /32

        # ---- BOTTLENECK (Inception) ----
        self.bottleneck = InceptionConvBlock(512, 512)

        # ---- DECODER (Inception + Spatial Attention) ----
        # UpBlock(in_ch, skip_ch, out_ch)
        self.d1 = UpBlock(512, 256, 256)   # s4 bottleneck  → attend s3 (256ch)
        self.d2 = UpBlock(256, 128, 128)   # → attend s2 (128ch)
        self.d3 = UpBlock(128,  64,  64)   # → attend s1  (64ch)
        self.d4 = UpBlock( 64,  64,  64)   # → attend s0  (64ch, stem)
    ...
    def forward(self, x):
        ...
        # Decoder (each UpBlock attends the skip before concat)
        x  = self.d1(b,  s3)
        x  = self.d2(x,  s2)
        x  = self.d3(x,  s1)
        x  = self.d4(x,  s0)
...
def main():
    ...
    model = ResNetInceptionUNet(pretrained=True).to(device)
